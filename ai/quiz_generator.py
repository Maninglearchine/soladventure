...
DIFF_DESC = {
    "easy":   "매우 쉬움 — 금융 개념의 뜻을 단순히 고르는 문제 (예: '저축이란 무엇일까요?')",
    "medium": "보통 — 개념을 일상 상황에 적용하는 문제 (예: '이럴 때 어떻게 하면 좋을까요?')",
    "hard":   "어려움 — 두 개념을 비교하거나 이유를 설명하는 문제 (예: '왜 분산투자가 중요할까요?') ※ 숫자 계산 불가",
}
...
class QuizGenerator:
    ...
    def _openai_quiz(self, world, concept, difficulty, age_group, character_name, enemy_name):
        ...
        _AGE_LABEL = {
            "young":       "7~9세 어린이 — 1학년~3학년 수준. 사용 가능 단어: 용돈·저금통·간식·장난감·돈·아끼다·모으다. 금융 용어 금지.",
        ...
        # 7~9세 전용 추가 지시 — 매우 구체적인 예시로 난이도 강제
        young_extra = ""
        if age_group == "young":
            young_extra = """
════ 7~9세 전용 추가 규칙 ════
• 질문은 딱 한 문장, 초등 1~2학년이 바로 이해할 수 있어야 함
• 선택지 텍스트는 5단어 이내의 짧은 구문
• 아래 예시 수준으로 만들 것:
  ✅ 좋은 예: "저금통에 돈을 모으는 것을 뭐라고 할까요?"
             → 선택지: "저축해요" / "다 써요" / "빌려요"
  ✅ 좋은 예: "용돈을 다 쓰지 않고 아끼는 건 왜 좋을까요?"
             → 선택지: "나중에 쓸 수 있어요" / "재미없어요" / "빨리 없어져요"
  ❌ 나쁜 예: "투자와 저축의 차이점은?" (너무 어려운 개념)
  ❌ 나쁜 예: "복리란 무엇인가요?" (금융 전문 용어)
• feedback도 짧게 — 한 문장, 7세 어린이가 이해할 수 있는 말
════════════════════════════════
"""
        ...
        system = f"""너는 어린이 금융 교육 게임 "쏠어드벤처"의 퀴즈 생성 AI야.

════ 절대 금지 (이 규칙을 어기면 생성 실패) ════
❌ 숫자가 포함된 계산 문제 — 금액·퍼센트·배율 계산, 이자·수익률 계산, 숫자 대소 비교 등 일체 금지
❌ 예시: "원금 10만 원에 이자율 5%이면 이자는?", "500원을 3일 모으면?" 같은 문제 절대 금지
❌ 투자 수익 보장 표현 절대 금지
════════════════════════════════════════════
{young_extra}
✅ 허용되는 문제 유형 (아래 형식만 사용):
- "~란 무엇일까요?" (개념 정의)
- "~을 하는 이유는?" (이유 설명)
- "~와 ~의 차이는?" (개념 비교)
- "이럴 때 가장 좋은 행동은?" (상황 판단)

대상 연령: {age_label}
핵심 개념 주제: {concept}
난이도: {diff_desc}
게임 배경: {character_name}이(가) {world_ko}에서 {enemy_name}을(를) 만났어!{stock_rule}
...
{f"참고 금융 개념:{chr(10)}{context}" if context else ""}

JSON 형식:
{{
  "question": "질문 (1문장, 세계 배경 포함, 숫자 계산 없음)",
  "choices": [
